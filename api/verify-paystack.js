...
// program mega-update; see nova-affiliate/ARCHITECTURE.md) ----
//
// index.html must send two extra fields when calling this endpoint,
// resolved client-side from localStorage right before checkout:
//   affiliateRef        the affiliate's uid, or null if no attribution
//   affiliateRefSource  'product' | 'general' | null
// See PATCH-index-html-attribution.js for the exact client-side snippet.
//
...
async function attributeAffiliateCommission(db, { reference, buyerUid, productId, productName, amountMajor, currency, affiliateRef, affiliateRefSource }) {
  ...
  if (!affSnap.exists || affSnap.data().status !== 'active') return;

  // General-link commissions only ever pay out ONCE per buyer (first
  // product bought after tapping the general store link) — authoritative
  // check lives server-side (users/{uid}.generalAffiliateConsumed) so a
  // cleared localStorage can't be used to farm repeat general-link payouts.
  if (affiliateRefSource === 'general') {
  ...
  // doc/field every other admin-editable setting in index.html uses.
  // No separate "affiliateSettings" doc.
  const settingsDoc = await getCatalogDoc('settings');
  const content = (settingsDoc && settingsDoc.content) || {};
  const defaultPct = typeof content.affiliateCommissionDefaultPct === 'number' ? content.affiliateCommissionDefaultPct : 10;
  const pct = productInfo.commissionPct != null ? productInfo.commissionPct : defaultPct;
  const commissionAmount = Math.round(amountMajor * pct) / 100;
  if (commissionAmount <= 0) return;

  try {
    // Doc ID = reference, same idempotency guarantee as the orders/{reference}
    // pattern above — safe if this ever runs twice for the same payment.
    // Commissions go straight to 'available' — the admin's manual
    // approve-before-payout step (Affiliate Program tab) is the fraud/
    // refund checkpoint, not a separate hold timer.
    await db.collection('affiliateCommissions').doc(reference).set({
    ...
    const displayName = affSnap.data().displayName || 'Affiliate';
    const maskedName = displayName.split(' ')[0] + (displayName.split(' ')[1] ? ' ' + displayName.split(' ')[1][0] + '.' : '');
    const linkKey = `${affiliateRef}_${affiliateRefSource === 'general' ? 'general' : productId}`;
    const dailyKey = `${affiliateRef}_${new Date().toISOString().slice(0, 10)}`;
    await Promise.all([
      db.collection('affiliates').doc(affiliateRef).update({
        balanceAvailable: FieldValue.increment(commissionAmount),
        totalEarned: FieldValue.increment(commissionAmount),
      }),
      // Per-link sales counter (My Store tab — "X clicks, Y sales" per link).
      db.collection('affiliateLinkStats').doc(linkKey).set({
        ref: affiliateRef, pid: affiliateRefSource === 'general' ? null : productId,
        sales: FieldValue.increment(1),
      }, { merge: true }),
      // Daily rollup (dashboard graph).
      db.collection('affiliateDailyStats').doc(dailyKey).set({
        ref: affiliateRef, date: new Date().toISOString().slice(0, 10),
        sales: FieldValue.increment(1), earnings: FieldValue.increment(commissionAmount),
      }, { merge: true }),
      // Minimal leaderboard mirror — name + totals ONLY, never payout
      // details (see firestore.rules — any signed-in affiliate can read
      // this collection for the Leaderboard tab).
      db.collection('affiliateLeaderboard').doc(affiliateRef).set({
        displayName: maskedName, totalEarned: FieldValue.increment(commissionAmount), salesCount: FieldValue.increment(1),
      }, { merge: true }),
      affiliateRefSource === 'general'
        ? db.collection('users').doc(buyerUid).set({ generalAffiliateConsumed: true }, { merge: true })
        : Promise.resolve(),
    ]);
  } catch (e) {
    console.error('attributeAffiliateCommission failed (non-fatal):', e.message);
  }
}
...
module.exports = async function handler(req, res) {
  ...
  // ---- AFFILIATE PROGRAM: attribute commission, same fire-and-forget
