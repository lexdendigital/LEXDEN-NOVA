...
// ---- Order/admin emails (fire only after a real verified+priced order is
// written below) — calls Brevo directly, inline, rather than importing
// send-email.js across an ESM/CommonJS boundary, so a template/env hiccup
// can never risk the payment-verification response itself. Never throws —
// callers wrap this in try/catch anyway, but a failed send here should
// never turn a successful payment into an error response.
async function sendBrevoTemplate(templateEnvKey, to, params) {
  const apiKey = process.env.BREVO_API_KEY;
  const templateId = Number(process.env[templateEnvKey]);
  if (!apiKey || !templateId) {
    console.warn(`Skipping email — ${!apiKey ? 'BREVO_API_KEY' : templateEnvKey} not set`);
    return;
  }
  try {
    ...
    if (!r.ok) console.error('Brevo send failed', templateEnvKey, r.status, await r.text().catch(()=>''));
  } catch (e) {
    console.error('Brevo send threw', templateEnvKey, e.message);
  }
}
...
export default async function handler(req, res) {
  ...
  // ---- Order confirmation (to shopper) + admin notification — both are
  // best-effort: awaited (a Vercel Node function can be frozen the instant
  // the response is sent, so "fire and forget without awaiting" risks the
  // email never actually going out) but wrapped so a Brevo hiccup never
  // turns a genuinely successful, already-saved payment into an error
  // response to the shopper.
  ...
  await Promise.all([
    buyerEmail ? sendBrevoTemplate('BREVO_TPL_ORDER', { email: buyerEmail }, {
      order_id: reference,
