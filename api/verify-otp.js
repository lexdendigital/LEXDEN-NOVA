...
// Same HMAC(secret, "<email>|<code>") shape send-otp.js now stores instead
// of a plaintext code — kept here too since verify-otp.js is the only
// other file that needs to reproduce it, and duplicating one small pure
// function is simpler than adding a shared module just for this.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // SECURITY FIX (roadmap 3.4): compare HMAC hashes with a constant-time
    // comparison instead of `data.code !== code` on a plaintext value —
    // send-otp.js no longer stores the plaintext code at all (see its
    // hashCode() comment), and a plain !== on secret-derived values is a
    // (minor, but free-to-fix) timing side channel besides.
    let hashOk = false;
    try {
      const submittedHash = hashCode(code, email);
      const storedHash = String(data.codeHash || "");
      const a = Buffer.from(submittedHash, "utf8"), b = Buffer.from(storedHash, "utf8");
      hashOk = a.length === b.length && crypto.timingSafeEqual(a, b);
    } catch (e) {
      console.error("verify-otp: hash check failed —", e.message);
